  ...
  public CuteGame(Platform plat) {
    ...
    plat.input().keyboardEvents.connect(new Keyboard.KeySlot() {
      @Override public void onEmit (Keyboard.KeyEvent event) {
        if (event.down) {
          ...
          if (tileIdx != null) {
            addTile((int) Marine.x, (int) Marine.y, tileIdx);
            return;
          }
        }
        ...
        switch (event.key) {
          case SPACE:
            ...
            break;
          case ESCAPE:
            if(event.down) removeTopTile((int) Marine.x, (int) Marine.y);
            break;
          case LEFT:
    ...
    rootLayer.add(gameLayer = new Layer() {
      @Override protected void paintImpl (Surface surface) {
        if (Marine != null) world.setViewOrigin(
          Marine.x(frameAlpha), Marine.y(frameAlpha), Marine.z(frameAlpha));
        surface.clear();
        world.paint(surface, frameAlpha);
      }
    });

    plat.assets().getImage("images/marine.png").state.onSuccess(new Slot<Image>() {
      public void onEmit (Image image) {
        Marine = new CuteObject(image.texture());
        Marine.setPos(2, 2, 1);
        Marine.r = 0.3;
        world.addObject(Marine);

        update.connect(new Slot<Clock>() {
          public void onEmit (Clock clock) {
            Marine.setAcceleration(0, 0, 0);

            if (Marine.isResting()) {
              // Keyboard control.
              if (controlLeft) Marine.ax = -1.0;
              if (controlRight) Marine.ax = 1.0;
              if (controlUp) Marine.ay = -1.0;
              if (controlDown) Marine.ay = 1.0;
              //if (controlShift) Marine.ay = 10.0;

              // Mouse Control.
              Marine.ax += touchVectorX;
              Marine.ay += touchVectorY;
